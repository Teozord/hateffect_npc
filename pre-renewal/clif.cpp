...
void clif_guild_allianceinfo(map_session_data& sd){
	...
	auto& g = (sd.bg_id ? *bg_guild_get(sd.bg_id):sd.guild->guild);
	if (!&g)
		return;
...
static void clif_guild_positioninfolist(map_session_data& sd){
	...
	for(size_t i=0;i<MAX_GUILDPOSITION;i++){
		guild_position& gp = g.position[i];
		...
		case BL_MOB: {
			...
			if( md->guardian_data && md->guardian_data->guild_id ){
				...
				if(battle_config.mob_ele_view)
				{
					...
					{
						case 0: ele_group = 51; strcpy(ele_name,"Neutro");	break;
						case 1: ele_group = 52; strcpy(ele_name,"Agua"); break;
						case 2: ele_group = 53; strcpy(ele_name,"Terra"); break;
						case 3: ele_group = 54; strcpy(ele_name,"Fogo"); break;
						case 4: ele_group = 55; strcpy(ele_name,"Vento"); break;
						case 5: ele_group = 59; strcpy(ele_name,"Veneno"); break;
						case 6: ele_group = 57; strcpy(ele_name,"Sagrado"); break;
						case 7: ele_group = 58; strcpy(ele_name,"Sombrio"); break;
						case 8: ele_group = 59; strcpy(ele_name,"Fantasma"); break;
						case 9: ele_group = 60; strcpy(ele_name,"Amorfo"); break;
						default: break;
					}
...
void clif_parse_GuildRequestEmblem(int32 fd,map_session_data *sd)
{
	...
	int32 guild_id = RFIFOL(fd,packet_db[RFIFOW(fd,0)].pos[0]),i;
...
int32 clif_skill_select_list(map_session_data *sd, map_session_data *tsd) {
	int32 i;
	int32 fd;
	uint16 skills[3];
...
int32 clif_visual_guild_id(struct block_list *bl)
{
	int32 bg_id;
	nullpo_ret(bl);
...
int32 clif_visual_emblem_id(struct block_list *bl)
{
	int32 bg_id;
	nullpo_ret(bl);
	if ((bg_id = bg_team_get_id(bl)) == 0)
...
void clif_hat_effect(struct block_list* bl, int16 effectID, bool enable) {
#if PACKETVER >= 20150507
    unsigned char buf[8];
    
    WBUFW(buf,0) = 0xa3b; // Packet ID para hat effect
    WBUFL(buf,2) = bl->id;
    WBUFW(buf,6) = enable ? effectID : 0;
    
    clif_send(buf, packet_len(0xa3b), bl, AREA);
#endif
}
...
void clif_parse_itempackage_select( int32 fd, map_session_data* sd ){
	...
	for( const auto& entry : group->items ){
		...
		if( entry.second->rentalhours ){
			item.expire_time = static_cast<uint32_t>(time(nullptr) + static_cast<uint64_t>(entry.second->rentalhours) * 3600);
		}
...
void clif_emotion_expansion_list(map_session_data *sd, std::vector<struct PACKET_ZC_EMOTION_EXPANTION_LIST_sub> &list)
{
	...
	size_t computedLength = sizeof(struct PACKET_ZC_EMOTION_EXPANTION_LIST) +
		sizeof(struct PACKET_ZC_EMOTION_EXPANTION_LIST_sub) * list.size();

	p->packetLength = (computedLength > std::numeric_limits<int16_t>::max())
		? std::numeric_limits<int16_t>::max()
		: static_cast<int16_t>(computedLength);
...
void clif_rank_info(map_session_data *sd, int32 points, int32 total, e_rank ranktype)
{
	char message[100];
...
void do_init_clif(void) {
	...
	add_timer_interval(
		gettick() + static_cast<int64_t>(battle_config.ping_timer_interval) * 1000,
		clif_ping_timer,
		0,
		0,
		static_cast<int64_t>(battle_config.ping_timer_interval) * 1000
	);
#endif
