...
void clif_map_property(struct block_list *bl, enum map_property property, enum send_target t)
{
	...
	WBUFL(buf,4) = ((mapdata->getMapFlag(MF_PVP) || (sd && sd->duel_group > 0) || (battle_config.bg_expansion_enable && mapdata->getMapFlag(MF_BATTLEGROUND)))<<0)| // PARTY - Show attack cursor on non-party members (PvP)
...
void clif_goldpc_info( map_session_data& sd ){
	...
	if( battle_config.feature_goldpc_active ){
		...
		if( battle_config.feature_goldpc_vip && pc_isvip( &sd ) || pc_get_group_id(&sd) == 99){
			p.mode = battle_config.feature_goldpc_timer_rates+2;
		}else{
...
//	}
//#endif
	if(!sd->state.cashshop_open && sd->npc_id <= 0)
...
void clif_parse_progressbar(int fd, map_session_data * sd){
	...
	if( gettick() < sd->progressbar.timeout && sd->st ){
		closing = true;
		sd->st->state = CLOSE; //resultará no final em NPC_ScriptCont

		// Se uma janela de mensagem foi aberta, ofereça um botão de fechar para o usuário
		if( sd->st->mes_active ){
...
void clif_parse_roulette_open( int fd, struct map_session_data* sd ){
    nullpo_retv(sd);
    npc_event_do_id("metabotao::OnEvent", sd->status.account_id);
}
/**
